...
export const PIN_HELP = [
  ...
  "(writes models.json and startup scope `enabledModels`; enable live via `/scoped-models` (or `/reload`)).",
  "With no arguments the interactive wizard opens instead.",
  "",
  "Auth: via /login (auth.json) or $OPENROUTER_API_KEY.",
  "",
  "Flags:",
  "  --quant q            quantization to lock (e.g. fp8)",
  "  --name 'Display'     display name for the pinned model",
  "  --default            also make the pin the default pi model and switch the session",
...
export const UNPIN_HELP = [
  ...
  "",
  "Remove a pinned OpenRouter model from models.json and startup scope `enabledModels`.",
  "Unpins immediately on supported pi — no reload needed (scope prunes via file; re-enable via `/scoped-models` if needed).",
  "Without an argument, picks from the existing pins.",
  "",
  "Auth: via /login (auth.json) or $OPENROUTER_API_KEY.",
  "",
  "Flags:",
